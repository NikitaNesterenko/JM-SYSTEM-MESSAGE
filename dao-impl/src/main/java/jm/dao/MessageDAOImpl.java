...
public class MessageDAOImpl extends AbstractDao<Message> implements MessageDAO {
    ...
    public Optional<LocalDateTime> getMessageDateCreateByMessageId(@NonNull Long messageId) {
        ...
        try {
            Timestamp timestampTime = (Timestamp) entityManager.createNativeQuery("SELECT m.date_create FROM messages m WHERE m.id= :messageId")
                    .setParameter("messageId", messageId).getSingleResult();
            dateCreate = timestampTime.toLocalDateTime();
        } catch (NoResultException ignored) {
