...
public class UserRestController {
    ...
    public Response<List<AssociatedUserDTO>> getAllUsersForMarked(@PathVariable("id") Long id) {
        /* TODO доделать логгирование*/
        Optional<List<AssociatedUserDTO>> list = userService.getAllAssociatedUserDTOinWorkspaceByWorkspaceId(id);
        return list.map(Response::ok).orElseGet(() -> Response.error(HttpStatus.BAD_REQUEST, "No users in workspace with such id"));
    }
