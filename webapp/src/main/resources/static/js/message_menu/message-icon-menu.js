...
import {ActiveChatMembers} from "../workspace-page/components/sidebar/ActiveChatMembers.js";
...
const activeChatMembers = new ActiveChatMembers();
...
$(document).on('click', '[id^=createOrShowConversation]', async function (e) {
    let associated_user_id = $(e.target).data('associated_user_id');
    let conversation = {
        id: 0,
        openingUser: await user_service.getLoggedUser(),
    ...
    await conversation_service.createOrShowConversation(conversation).then(status => {
        if (status === 200) {
            ...
            console.log("Error occurred while creating new direct message conversation")
        }
    });
});
