export class MessageDialogView {
    ...
    container(message) {
        ...
        this.root.append(this.getMessageMenuIcons());
        ...
        return `
            <div class="message-icons-menu-class" id="message-icons-menu">
                <div class="btn-group" role="group" aria-label="Basic example">
                    <button type="button" class="btn btn-light">${this.ICONS.EMOJI_BUTTON}</button>
                    <button type="button" class="btn btn-light">${this.ICONS.REPLY_BUTTON}</button>
                    <button type="button" class="btn btn-light" id="share-message-id" data-msg_id="${this.message.id}">${this.ICONS.SHARE_BUTTON}</button>
                    <button id="msg-icons-menu__starred_msg_${this.message.id}" data-msg_id="${this.message.id}" type="button" class="btn btn-light">${this.ICONS.STAR_BUTTON_BLANK}</button> 
                    <button type="button" class="btn btn-light" name="btnEditInline" data-msg-id=${this.message.id} data-user-id=${this.message.user === null ? '' : this.message.user.id}>${this.ICONS.MSG_EDIT_BUTTON}</button>
                </div>
            </div>
        `;
    }
    ...
    setSharedMessage(message) {
        const user_name = message.user === null ? message.bot.name : message.user.name;
        this.root.find(".c-message__content--feature_sonic_inputs").append(`
    ...
    getMessageUserId() {
        return this.message.user === null ? this.message.bot.id : this.message.user.id;
    }

    getDataUserIdAttr() {
        if (this.message.user) {
            return `data-user_id="${this.message.user.id}"`;
        }
        return `data-bot_id="${this.message.bot.id}"`;
    }
}